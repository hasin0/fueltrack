    ...
    <div class="card-body">
        ...
        <form method="post" action="{{route('fuelrequests.update', $fuelrequests->id)}}">
            ...
            <div class="form-group">
                ...
                <input id="ltr_collected" type="number" name="ltr_collected" placeholder="Enter Last Fuel Quantity"  value="{{ old('ltr_collected', $fuelrequests->ltr_collected) }}" class="form-control">
                @error('ltr_collected')
                    <span class="text-danger">{{$message}}</span>
                @enderror
            </div>
                ...
                <select name="vehicle_id" class="form-control">
                    ...
                    @foreach($vehicle as $vehicles)
                        <option value="{{$vehicles->id}}" {{ (old('vehicle_id', $fuelrequests->vehicle_id) == $vehicles->id) ? 'selected' : '' }}>
                            Car-Tag {{$vehicles->tag_no}} :{{$vehicles->fueltank}} Liters : {{$vehicles->department}}
                        </option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <select name="department_id" class="form-control">
                    @foreach($department as $dept)
                        <option value="{{$dept->id}}" {{ (old('department_id', $fuelrequests->department_id) == $dept->id) ? 'selected' : '' }}>
                            {{$dept->name}}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select name="fuelstation_id" class="form-control">
                    ...
                    @foreach($fuelstation as $station)
                        <option value="{{$station->id}}" {{ (old('fuelstation_id', $fuelrequests->fuelstation_id) == $station->id) ? 'selected' : '' }}>
                            {{$station->name}}
                        </option>
                    @endforeach
                </select>
